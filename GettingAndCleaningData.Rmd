...
# Put data into a data frame or data table if very large, then look at the data 
gpd.r <- data.frame(read.csv("./data/gpd", header = TRUE))
education.r <- data.frame(read.csv("./data/education", header = TRUE))
glimpse(gpd.r)
glimpse(education.r)
colnames(gpd.r)[1] = "CountryCode"
head(gpd.r)
head(education.r)


#set up and execute the merge() function
gpd_education <- merge(gpd.r, education.r, by = "CountryCode", sort = TRUE )
gpd_education$Gross.domestic.product.2012 <- as.numeric(gpd_education$Gross.domestic.product.2012)
gpd_education <- arrange(gpd_education, desc(Gross.domestic.product.2012))
glimpse(gpd_education)
str(gpd_education)
gpd_education[1:50,]
view(gpd_education)
